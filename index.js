...
console.log(chalk.cyan(figlet.textSync("ZORD TEAMS", {
    font: 'DOS Rebel',
...
✉️  Script Name : ZORD TEAMS V1
✉️  Author : ZORD TEAMS
✉️  Gmail :thri.iz.kidd@gmail.com
✉️  Instagram : thri.iz.kidd

${chalk.green.bold("Powered By ZORD TEAMS V1")}\n`));
...
async function startXliconBot() {
    ...
    const { state, saveCreds } = await useMultiFileAuthState(`./ZORD_TEAMS-SESSION`);
    ...
    if (pairingCode && !XliconBotInc.authState.creds.registered) {
        ...
        let phoneNumber;
        phoneNumber = await question('Please enter your number starting with 263 :\n');
        phoneNumber = phoneNumber.trim();

        setTimeout(async () => {
            const code = await XliconBotInc.requestPairingCode(phoneNumber);
            console.log(chalk.black(chalk.bgGreen(`🎁  Your Zord Teams Pairing Code : ${code}`)));
        }, 3000);
    }
